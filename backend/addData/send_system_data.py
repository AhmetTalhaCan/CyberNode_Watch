...
def load_config():
    ...
    default_config = {
        "server_url": "http://127.0.0.1:8080/receive_device_info",
        "desktopName": None  # Dinamik olarak belirlenecek
    }
    ...
    # desktopName her durumda dinamik olarak oluşturulsun
    config["desktopName"] = f"{platform.node()}-{os.getlogin()}"
    return config
...
def list_installed_software_with_version():
    ...
    for line in lines[1:]:
        # Satırı en az 2 sütuna ayırmaya çalış (Name ve Version)
        parts = line.strip().rsplit('  ', 1)  # Son boşluklardan 2 parçaya ayır
        if len(parts) == 2:
            name = parts[0].strip()
...
def run_agent(config):
    ...
    send_data_to_server(addData, config["server_url"])
